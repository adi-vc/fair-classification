...
class LinearClf():
    ...
    def fit(self, X, y, x_sensitive, cons_params=None):
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                w[k] = cp.Variable(X.shape[1]) # this is the weight vector
                w[k].value = np.random.rand(X.shape[1]) # initialize the value of w -- uniform distribution over [0,1]
        else:
            w = cp.Variable(X.shape[1]) # this is the weight vector
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                ...
                X_k = X[idx]
                y_k = y[idx]
                obj += cp.sum_squares(w[k][1:]) * self.lam[k] # first term in w is the intercept, so no need to regularize that

                if self.loss_function == "logreg":
                    obj += cp.sum(  cp.logistic( cp.multiply(-y_k, X_k*w[k]) )  ) / num_all # notice that we are dividing by the length of the whole dataset, and not just of this sensitive group. this way, the group that has more people contributes more to the loss
                    
                elif self.loss_function == "svm_linear":
                    obj += cp.sum ( cp.maximum (0, 1 - cp.multiply ( y_k,  X_k*w[k])) ) / num_all
            ...
            obj = 0
            obj += cp.sum_squares(w[1:]) * self.lam # regularizer -- first term in w is the intercept, so no need to regularize that
            if self.loss_function == "logreg":
                obj += cp.sum(  cp.logistic( cp.multiply(-y, X*w) )  ) / num_all
            elif self.loss_function == "svm_linear":
                obj += cp.sum ( cp.maximum (0, 1 - cp.multiply ( y,  X*w)) ) / num_all
            else:
                raise Exception("Invalid loss function")
    ...
    def get_di_cons_single_boundary(self, X, y, x_sensitive, w, cov_thresh):
        ...
        fx = X*w
        prod = cp.sum( cp.multiply(z_i_z_bar, fx) ) / X.shape[0]
        ...
        if cons_type in [1,2,3]: # 1 - pref imp, 2 - EF, 3 - pref imp & EF
            ...
            for val in set(x_sensitive):
                ...
                for k in list(w.keys()): # get the distance with each group's w
                    prod_dict[val][k] = cp.sum(  cp.maximum(0, X_g*w[k])   ) / num_g
